...
const DetailNotification: React.FC<DetailNotificationProps> = ({ onClose }) => {
  ...
  const validate = (values: number): string => {
    var errors: string = "";
    if (values < 10000) {
      errors = intl.formatMessage({ id: 'BalancePage.DetailNotification.text3' });
    }
    if (!values) {
      errors = intl.formatMessage({ id: 'BalancePage.DetailNotification.text6' });
    }
    return errors;
  };
  ...
  return (
    ...
    >
      <motion.div
        ref={notificationRef}
        className={`relative w-5/6 sm:w-1/2 bg-white rounded-xl p-4 border-2 border-red-500 overflow-y-auto
      ...
      >
        <div className="relative items-center flex-col flex h-10 w-full border-b-2 border-gray-300">
          <div className="font-bold text-xl">Thông tin lô hàng</div>
        ...
        <div className={`relative self-center w-full mt-6 pb-6 border-b-2 border-gray-300`}>
          ...
          >
            <FormattedMessage id="BalancePage.DetailNotification.text2"/>
          </label>
        ...
        <div className="mt-2 pb-4 border-b-2 border-gray-300">
          <p className="pl-1"><FormattedMessage id="BalancePage.DetailNotification.text4"/></p>
